...
let BncrHost = "http://192.168.3.2:9090";
let BncrToken = "dada575688";
let PushErrorTime = 0;
let strTitle = "";
...
const { getEnvs, DisableCk, getEnvByPtPin } = require("./ql");
const fs = require("fs");
...
async function sendNotify(text, desp, params = {}, author = "\n\n本通知 By ccwav Mod", strsummary = "") {
    ...
    try {
        ...
        if (strTitle == "汪汪乐园养joy领取" && WP_APP_TOKEN_ONE) {
            ...
            var strdecPtPin = decodeURIComponent(strPtPin);
            if (strPtPin) {
                await sendNotifybyWxPucher(
                    "汪汪乐园领取通知",
                    `【京东账号】${strdecPtPin}\n当前等级: 30\n请自行去解锁新场景,奖励领取方式如下:\n极速版APP->我的->汪汪乐园,点击左上角头像，点击中间靠左的现金奖励图标，弹出历史奖励中点击领取.`,
                    strdecPtPin
                );
            }
        }
        ...
        if (process.env["TG_PROXY_AUTH" + UseGroupNotify] && Use_tgBotNotify) TG_PROXY_AUTH = process.env["TG_PROXY_AUTH" + UseGroupNotify];
        if (process.env["TG_PROXY_HOST" + UseGroupNotify] && Use_tgBotNotify) TG_PROXY_HOST = process.env["TG_PROXY_HOST" + UseGroupNotify];
        if (process.env["TG_PROXY_PORT" + UseGroupNotify] && Use_tgBotNotify) TG_PROXY_PORT = process.env["TG_PROXY_PORT" + UseGroupNotify];
        if (process.env["TG_API_HOST" + UseGroupNotify] && Use_tgBotNotify) TG_API_HOST = process.env["TG_API_HOST" + UseGroupNotify];
...
function Env(t, s) {
    return new (class {
        ...
        lodash_set(t, s, e) {
            return Object(t) !== t
                ...
                : (Array.isArray(s) || (s = s.toString().match(/[^.[\]]+/g) || []),
                  (s.slice(0, -1).reduce((t, e, i) => (Object(t[e]) === t[e] ? t[e] : (t[e] = Math.abs(s[i + 1]) >> 0 == +s[i + 1] ? [] : {})), t)[
                      s[s.length - 1]
                  ] = e),
                  t);
        }
        ...
        time(t) {
            ...
            for (let e in s)
                new RegExp("(" + e + ")").test(t) &&
                    (t = t.replace(RegExp.$1, 1 == RegExp.$1.length ? s[e] : ("00" + s[e]).substr(("" + s[e]).length)));
            return t;
        }
        msg(s = t, e = "", i = "", o) {
            const h = (t) =>
